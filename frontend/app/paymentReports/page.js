...
const PaymentList = () => {
    ...
    useEffect(() => {
        const fetchPayments = async () => {
            try {
                setLoading(true);
                const response = await axios.get('http://ec2-13-232-147-76.ap-south-1.compute.amazonaws.com:5000/api/payment/paymentsList');
                const paymentsWithTableNames = await Promise.all(
                    response.data.map(async (payment) => {
                        const tableResponse = await axios.get(`http://ec2-13-232-147-76.ap-south-1.compute.amazonaws.com:5000/api/table/tables/${payment.tableId}`);
                        return {
                            ...payment,
